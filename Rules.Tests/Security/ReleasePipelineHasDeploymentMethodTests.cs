...
namespace SecurePipelineScan.Rules.Tests.Security
{
    public class ReleasePipelineHasDeploymentMethodTests : IClassFixture<TestConfig>
    {
        ...
        public async Task WhenValidUserThenCanUpdateDeploymentInfo()
        {
            ...
            var assignment = new AssignmentContentItem { Assignment = new Assignment { Operators = new List<string> { "x.y@rabobank.nl" } } };
            ...
            vstsClient.GetAsync(Arg.Any<IVstsRequest<UserEntitlement>>()).Returns(Task.FromResult(new UserEntitlement { User = new User { PrincipalName = "x.y@rabobank.nl" } }));
        ...
        public async Task WhenAzDoNonProdPipelineThenSkipsAuthorization()
        {
            ...
            cmdbClient.Config.Returns(new CmdbClientConfig("abc", "https://localhost", "raboweb", "CI1234"));

            var assignment = new AssignmentContentItem { Assignment = new Assignment { Operators = new List<string> { "x.y@rabobank.nl" } } };
            cmdbClient.GetAssignmentAsync("TAS").Returns(Task.FromResult(assignment));

            var vstsClient = Substitute.For<IVstsRestClient>();
            vstsClient.GetAsync(Arg.Any<IVstsRequest<UserEntitlement>>()).Returns(Task.FromResult(new UserEntitlement { User = new User { PrincipalName = "notautorised@rabobank.nl" } }));
            var rule = new ReleasePipelineHasDeploymentMethod(vstsClient, cmdbClient);
        ...
        {
            // Arrange
            var supplementaryInfo = new SupplementaryInformation { Organization = "raboweb", Project = "1", Pipeline = "2" };
